...
void draw_particles(ui_t *ui, game_t *game, int x, int y)
{
    ...
    int a = game->back_card_color[0];
    int b = game->back_card_color[1];
    int c = game->back_card_color[2];
...
void draw_steal(ui_t *ui, game_t *game)
{
    ...
    if (ui->ticks_stealing_init==0){
        ...
        for (int i = 0; i < number_cards_stolen * 10; i += 10)
        {
            ...
            draw_face(ui, game->face_card_color, param * finx + (100 - param) * debx, param * finy + (100 - param) * deby +i*20);
        }
